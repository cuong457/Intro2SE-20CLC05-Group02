...
const orders = [
  ...
  {
    id: 3,
    buyername: "Phạm Nguyễn Cao Cường",
    addr: "Đại học KHTN",
    total: "1.000",
  },
...
const SellerPage = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      setPageCount(
        data.length % 9 !== 0
          ? Math.floor(data.length / 9) + 1
          : Math.floor(data.length / 9)
      );
    };
  ...
  const changepagenumber = (index, total) => {
    if (index > 0 && index <= total) {
      ...
      if (foods.length <= end) end = foods.length;
      for (; start < end; start++) {
        newFoodList.push(foods[start]);
      }
  ...
  const createPageNumber = (total) => {
    if (total > 1) {
      ...
      pageNumBtn.push(
        <span className="pe-1" key="prev">
          <button
            ...
            onClick={() => changepagenumber(current_page - 1, total)}
          >
      ...
      for (let i = 1; i <= total && numberOfNBTN < MAX_PAGENUMBER_SHOW; i++) {
        let class_name = "page-number-btn";
        if (i === current_page) class_name += " page-number-btn-active";
        pageNumBtn.push(
          <span className="pe-1" key={i}>
            <button
              type="button"
        ...
        ++numberOfNBTN;
        if (
          numberOfNBTN === MAX_PAGENUMBER_SHOW &&
          current_page >= i &&
          i + 1 <= total
        ) {
          pageNumBtn.splice(1, 1);
  ...
  return (
    <div class="bg-white">
      <div class="container py-5">
        <div className="">
          <p class="mb-0 h1">Seller Home</p>
          <p class="me-title opacity-75">
            View food list, edit, add, delete item
          </p>
          <hr />
        </div>
        {/* Side Bar */}
        <div class="row mt-4">
          <div class="col-12 col-md-3">
            <div class="list-group list-group-flush gap-5">
              ...
              <Link
                to="/seller"
                class="list-group-item list-group-item-action nav-item d-flex align-content-center justify-content-between"
              >
                <div class="">
                  <i class="fa fa-caret-right" aria-hidden="true"></i>
                  <span class="ps-4 me-title">Store</span>
                </div>
                <span class="badge badge-pill bg-danger float-end text-center">
                  {foods.length}
                </span>
              </Link>
              {/* Tới trang đơn đặt hàng cùng thông báo */}
              ...
              <div class="list-group-item nav-item">
                ...
                <div class="collapse" id="Profilecollapse">
                  <Link
                    to="/seller/profile"
                    class="list-group-item list-group-item-action nav-item d-flex align-content-center justify-content-between border-0"
                  >
                    <span class="ps-4 me-title">Seller Profile</span>
                  </Link>
                  <Link
                    to="/seller/storeprofile"
                    class="list-group-item list-group-item-action nav-item d-flex align-content-center justify-content-between border-0"
                  >
                    <span class="ps-4 me-title">Store Profile</span>
                  </Link>
          ...
          <div class="col-12 col-md-9">
            <div class="card">
              ...
              {/* Thanh đổi trang */}
              <ul class="row pb-4">{createPageNumber(page_count)}</ul>
            </div>
          </div>
        </div>
      </div>
